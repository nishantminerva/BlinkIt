...
//  BlinkIt-Assignmnet
//
//  Created by Nishant Minerva on 11/08/24.
//
...
struct Experiment: View {
    ...
    @State private var hasTriggeredDownwardRefresh = false
    @Environment(\.refresh) private var refresh
    @State private var isCurrentlyRefreshing = false
    @State private var isCurrentlyRefreshingB = false
    let amountToPullBeforeRefreshing: CGFloat = 120
    let amountToPullBeforeRefreshingB: CGFloat = 120
    
    @State var current = 0
    
    func goUpward() async {
        try? await Task.sleep(nanoseconds: 3_000_000_000)
        if current < productsList.count && current > 0 {
            current = current - 1
            print(current)
        }
    ...
    func goDownward() async {
        try? await Task.sleep(nanoseconds: 3_000_000_000)
        if current < productsList.count - 1 {
            current = current + 1
            print(current)
        }
                ...
                VStack{
                    if isCurrentlyRefreshing {
                        VStack {
                            if current > 0 {
                                ProgressView()
                                Text("Moving to \(productsList[current - 1].categoryName)")
                            }
                        }
                    }
                    ScrollView {
                        ...
                        .overlay(GeometryReader { geo in
                            ...
                            if currentScrollViewPosition < -amountToPullBeforeRefreshing && !isCurrentlyRefreshing {
                                ...
                                Color.clear.preference(key: ViewOffsetKey.self, value: geo.frame(in: .named("scrollView")).maxY)
                            }
                        })
                    }
                    ...
                    .onPreferenceChange(ViewOffsetKey.self) { scrollPosition in
                        ...
                        let pullUpDistance = scrollPosition - scrollViewHeight
                        
                        if pullUpDistance >= 0 {
                            hasTriggeredDownwardRefresh = false
                        }
                        
                        if pullUpDistance < -amountToPullBeforeRefreshingB && !isCurrentlyRefreshing && !hasTriggeredDownwardRefresh {
                            isCurrentlyRefreshingB = true
                            hasTriggeredDownwardRefresh = true
                            Task {
                                await goDownward()
                                await MainActor.run {
                                    isCurrentlyRefreshingB = false
                                    hasTriggeredDownwardRefresh = false
                                }
                            }
                        }
    ...
    func CustomVerticalTab() -> some View {
        ScrollViewReader { proxy in
            ScrollView(showsIndicators: false) {
                VStack(spacing: 5) {
                    ForEach(0..<productsList.count) { index in
                        ZStack(alignment: .trailing) {
                            VStack {
                                Image(categories[index][1])
                                    ...
                                    .background(current == index ? Color.green.opacity(0.5) : Color.gray.opacity(0.1))
                                    .clipShape(Circle())
                                Text(categories[index][0])
                                    .font(.system(size: 8))
                                    .multilineTextAlignment(.center)
                                    .foregroundStyle(current == index ? Color.black : Color.gray)
                            }
                            ...
                            .id(index) // Set ID for each tab for scrollTo functionality
...
// Experimenting to achieve both down pull and up pull
struct BothWay: View {
    @Environment(\.refresh) private var refresh
    @State private var isCurrentlyRefreshing = false
    @State private var isCurrentlyRefreshingB = false
    let amountToPullBeforeRefreshing: CGFloat = 180
    let amountToPullBeforeRefreshingB: CGFloat = 180
    ...
    var body: some View {
        VStack{
            ...
            .onPreferenceChange(ViewOffsetKey.self) { scrollPosition in
                ...
                if pullUpDistance < -amountToPullBeforeRefreshingB && !isCurrentlyRefreshing {
                    isCurrentlyRefreshingB = true
                    Task {
                        await refreshData()
                        await MainActor.run {
...
var categories = [
    ...
    ["All", "f1"],["Fresh \nVegetables", "f2"],["Fresh \nFruits", "f3"],["Flowers \n& Leaves", "f1"],["Corlandar \n& Others", "f2"],["Excotics", "f3"],["Mangoes & Melons", "f1"],["Seasonal", "f2"],["Freshly Cut \n& Sprouts", "f3"],["Frozen \nVeg", "f1"],
